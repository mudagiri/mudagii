...
import { categoryDiagnosis } from './mudagiri-diagnosis-v2';
...
function judgeCommunication(amount: number): BattleBranch {
  // Production Diagnosis Engine V2.0 is the single source of truth.
  // Mobile is an optimization category: max(actual - 4,000, 0) × 80%.
  // A valid mobile amount therefore never becomes `unknown`; unknown is reserved
  // for categories/inputs that genuinely require additional appraisal.
  const result = categoryDiagnosis({
    category: 'mobile',
    actual: amount,
    comparable: null,
  });
  if (result.needsReview) return 'unknown';
  return result.reducible > 0 ? 'defeat' : 'spare';
}

function BattleScene({ amount, household: _household }: { amount: number; household: FamilyProfile; }) {
  const diagnosis = useMemo(() => categoryDiagnosis({ category: 'mobile', actual: amount, comparable: null }), [amount]);
  const branch = judgeCommunication(amount);
  type BattlePhase = 'trace'|'reveal'|'normal'|'windup'|'slash'|'impact'|'knockback'|'defeated'|'dissolve'|'escape'|'obscure'|'result';
  const [phase, setPhase] = useState<BattlePhase>('trace');

  useEffect(() => {
    setPhase('trace');
    const schedule: Array<[number, BattlePhase]> = [
      [700, 'reveal'],
      [1500, 'normal'],
      [2600, 'windup'],
    ];
    if (branch === 'defeat') {
      schedule.push([2800,'slash'],[2920,'impact'],[3020,'knockback'],[3120,'defeated'],[3700,'dissolve'],[4450,'result']);
    } else if (branch === 'spare') {
      schedule.push([2900,'escape'],[4050,'result']);
    } else {
      schedule.push([2900,'obscure'],[3950,'result']);
    }
    const timers = schedule.map(([ms,next]) => window.setTimeout(() => setPhase(next), ms));
    return () => timers.forEach(window.clearTimeout);
  }, [branch]);

  const isTrace=phase==='trace', isReveal=phase==='reveal', isNormal=phase==='normal';
  const isWindup=phase==='windup', isSlash=phase==='slash', isImpact=phase==='impact';
  const isKnockback=phase==='knockback', isDefeated=phase==='defeated', isDissolve=phase==='dissolve';
  const isEscape=phase==='escape', isObscure=phase==='obscure', isResult=phase==='result';
  const showDefeated = branch==='defeat' && (isDefeated || isDissolve || isResult);
  const showEscape = branch==='spare' && (isEscape || isResult);
  const enemySrc = showDefeated ? `${BATTLE_ASSET}/TSUSHIN_DEFEATED.png` : showEscape ? `${BATTLE_ASSET}/TSUSHIN_ESCAPE.png` : `${BATTLE_ASSET}/TSUSHIN_NORMAL.png`;
  ...
  const resultSub = branch==='defeat' ? `月あたり約¥${diagnosis.reducible.toLocaleString('ja-JP')}の改善余地を検出。家計全体の解析はまだ続いています` : branch==='spare' ? '必要な支出まで無理に斬りません' : '追加情報があれば、さらに判定できます';
  ...
  return (
    <div className={`battle-scene battle-${branch} battle-phase-${phase}`}>
      ...
      {isReveal && <img className="battle-fx battle-fx-reveal" src={`${BATTLE_ASSET}/FX-01_REVEAL.png`} alt="" aria-hidden="true" />}
      {isSlash && <img className="battle-fx battle-fx-slash" src={`${BATTLE_ASSET}/FX-02_SLASH.png`} alt="" aria-hidden="true" />}
      {isImpact && <><div className="battle-white-flash" aria-hidden="true"/><img className="battle-fx battle-fx-hit" src={`${BATTLE_ASSET}/FX-03_HIT.png`} alt="" aria-hidden="true"/></>}
      {isDissolve && <img className="battle-fx battle-fx-particles" src={`${BATTLE_ASSET}/FX-04_DEFEAT_PARTICLES.png`} alt="" aria-hidden="true"/>}
      {isEscape && <img className="battle-fx battle-fx-dust" src={`${BATTLE_ASSET}/FX-05_ESCAPE_DUST.png`} alt="" aria-hidden="true"/>}
      {isObscure && <img className="battle-fx battle-fx-unknown" src={`${BATTLE_ASSET}/FX-06_UNKNOWN.png`} alt="" aria-hidden="true"/>}
